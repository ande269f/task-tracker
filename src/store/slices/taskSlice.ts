...
const inputSlice = createSlice({
    ...
    reducers: {
        ...
        setTaskCompleted: (state, action: PayloadAction<{taskUuid: UUIDTypes; taskCompleted: boolean}>) => {
            const findTask = state.find(t => t.taskUuid === action.payload.taskUuid);
                if (findTask) {
                    findTask.taskCompleted = action.payload.taskCompleted
                }
        },
        setTaskEditsLog: (state, action: PayloadAction<{taskUuid: UUIDTypes; taskEditsLog: {dateEdited: Date; taskText: string; taskCompleted: boolean; taskDeleted: Date | null, taskEditsUuid: UUIDTypes, taskUuid: UUIDTypes}}>) => {
            const findTask = state.find(t => t.taskUuid === action.payload.taskUuid);
            if (findTask) {
        ...
        deleteTask: (state, action: PayloadAction<{uuid: UUIDTypes}>) => {
            const updated = [...state]
            const taskIndex = state.findIndex(t => t.taskUuid === action.payload.uuid);
